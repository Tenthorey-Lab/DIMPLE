...
def run():
    if not any([app.delete.get(), app.insert.get(), app.include_substitutions.get(), app.dis.get(), app.duplicate.get()]):
    ...
    if app.phaseshift.get():
        phase_extension = 2
    else:
        phase_extension = 0
    if app.delete.get():
        overlapR = max([int(x) for x in app.deletions.get().split(',')]) + overlapR - 3 + phase_extension
    ...
    DIMPLE.phaseshift = app.phaseshift.get()
    ...
    if app.duplicate.get() == 0:
        duplications = False
    else:
        duplications = [int(x) for x in app.duplications.get().split(',')]
    generate_DMS_fragments(OLS, overlapL, overlapR, app.synonymous.get(), app.custom_mutations, app.include_substitutions.get(), insertions, duplications, deletions, app.dis.get(), app.wDir)
...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.duplicate = tk.IntVar()
        self.phaseshift = tk.IntVar()
        self.stop = tk.IntVar()
        ...
        self.duplicate_check = tk.Checkbutton(self, text="List of Duplications", variable=self.duplicate)
        self.duplicate_check.pack()
        self.duplicate_check.deselect()
        self.duplications = tk.Entry(self, width=50, textvariable=tk.StringVar(self, '3,6'))
        self.duplications.pack()

        self.phaseshift_check = tk.Checkbutton(self, text="Include Phase-Shifted Indels", variable=self.phaseshift)
        self.phaseshift_check.pack()
